<?php
	...
	$page_title = "English in Dublin | 게임 | SUDOKU";
...
<?php
	...
	{
		echo'
		<div  style="border:0px solid #ccc;" class="info_block">
	...
	function show_game($container = array(), $map = array(), $errors)
	{
		...
		<input type="submit" name="check" value="끝" /><br>
		<input type="reset" name="reset" value="다시 시작"/>
		</form>
		<form action="" method="post" style=" padding:0; margin-bottom:30px; margin-top:30px; border:0px solid #ccc;">
		<input type="hidden" name="token" value="'.$_SESSION['token'].'"/>
		<input type="submit" name="refresh_game" value="새 게임" style="margin-top:0;"/>
		</form>
		<a href="sudoku.php" style="font-size:12px;">다른 레벨 정하기..</a>';
	...
	function echoLevel()
	{
		...
		<p style="color:#09c; text-align:left;"><img style="float:left; margin-right:20px;" src="../img/sudoku_icon.jpg" />
			저희 온라인 수도쿠 게임을 사용해 주셔서 감사합니다. 레벨을 정해주세요.
		</p>
		</div>
		<div style="width:700px; padding:0 0 150px 0; margin:0 auto 0 auto; border:0px solid #ccc;">
			<form action="" method="post">
			<input type="hidden" name="token" value="'.$_SESSION['token'].'"/>
			<input type="submit" name="level" value="하급"/>
			<input type="submit" name="level" value="중급"/>
			<input type="submit" name="level" value="상급"/>
			</form>	
		</div>
		</div>
		';
