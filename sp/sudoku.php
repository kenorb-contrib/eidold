<?php
	...
	$page_title = "Escuela de Inglés | juego";
...
<?php
				...
				<table class="info_table" cellspacing="0px">
					<tr>
						...
						<td  style="background-color:#fff; "class="text_td">
							<p style="font-size:18px; font-weight:bold; padding-top:20px;">Disculpa</p>
							
							<p>
							Tienes que iniciar sesión antes de hacer nuestro test gratuito en <a href="login.php">línea</a>. (¿No eres usuario?, <a href="user_register.php">regístrate</a>)
							</p>
							
							 <p>
							 Si usted tiene alguna pregunta, estamos más que encantados de ayudarle. Simplemente <a href="contact_us.php">contáctenos</a>!
	...
	function show_game($container = array(), $map = array(), $errors)
	{
		...
		<div  style="border:0px solid #ccc; height:450px;" class="info_block">
		...
		<input type="submit" name="check" value="查看结果" /><br>
		<input type="reset" name="reset" value="清空"/>
		</form>
		<form action="" method="post" style=" padding:0; margin-bottom:30px; margin-top:30px; border:0px solid #ccc;">
		<input type="hidden" name="token" value="'.$_SESSION['token'].'"/>
		<input type="submit" name="refresh_game" value="新游戏" style="margin-top:0;"/>
		</form>
		<a href="sudoku.php" style="font-size:12px;">选择级别.</a>';
		$_SESSION['game'] .= '</div></div>';
		echo $_SESSION['game'];
	}
	...
	function echoLevel()
	{
		...
		<p style="color:#09c; text-align:left;"><img style="float:left; margin-right:20px;" src="../img/sudoku_icon.jpg" />
			感谢您抽空浏览我校网页，</br>请选择级别.
		</p>
		</div>
		<div style="width:700px; padding:0 0 150px 0; margin:0 auto 0 auto; border:0px solid #ccc;">
			<form action="" method="post">
			<input type="hidden" name="token" value="'.$_SESSION['token'].'"/>
			<input type="submit" name="level" value="容易"/>
			<input type="submit" name="level" value="中级"/>
			<input type="submit" name="level" value="高级"/>
			</form>	
		</div>
		</div>
		';
